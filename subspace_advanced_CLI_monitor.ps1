		...
		for ($arrPos = 0; $arrPos -lt $_farmers_ip_arr.Count; $arrPos++)
		{
			if ($_farmers_ip_arr[$arrPos].toString().Trim(' ') -ne "") {
				...
				if ($_process_type.toLower() -eq "node" -or $_process_type.toLower() -eq "farmer") { 
					...
					$_b_process_running_ok = fGetProcessState $_process_type $_host_url $_hostname "https://discord.com/api/webhooks/1202363726062162002/1RyMXertEPetP7YsTH298yH2xtoxgl_9kb4qenneNluwIPBVdEGDVrIQEWZDIT8L6tzb"
