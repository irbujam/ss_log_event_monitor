...
function fInvokeHttpRequestListener ([array]$_io_farmers_ip_arr, [object]$_io_context_task, [object]$_io_alert_stopwatch) {
	...
	while (!($_context_task.AsyncWaitHandle.WaitOne(200))) { 
			...
			$_prompt_listening_mode = "Listening on url: " + $_url_prefix_listener + "summary"
	...
	#$_html_full = fBuildHtml $_io_farmers_ip_arr $_io_alert_stopwatch
	...
	foreach ($_process_farm_sub_header in $_process_sub_header_arr)
	{
		...
		if ($_process_farm_sub_header.TotalSectors -ne "-")
		{
			...
			#
			if ($_process_farm_sub_header.RemainingSectors -ne "-" -and $_process_farm_sub_header.MinutesPerSectorAvg -ne "-") {
				$_process_eta = [math]::Round((([double]($_process_farm_sub_header.MinutesPerSectorAvg) * $_process_farm_sub_header.RemainingSectors)) / ($_process_farm_sub_header.TotalDisks * 60 * 24), 2)
...
#				<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.8.0/Chart.min.js"></script>
	$_html_full +=
				...
				//function fBarChartClick() {
				//   document.getElementById("progress").innerHTML = "' + "Hello" + '";
				//}
				//function fPieChartClick() {
				//   document.getElementById("rewards").innerHTML = "' + "Hello" + '";
				//}
				</script>'

	$_html_bar_chart = fBuildBarChart $_chart_labels $_chart_alt_labels $_chart_progess_data $_chart_eta_data $_chart_size_data $_chart_uptime_data $_chart_perf_sectorsPerHour_data $_chart_perf_minutesPerSector_data $_process_disk_data_js_arr 'Farm Plotting Progress'
	$_html_radar_chart = fBuildRadarChart $_chart_labels $_chart_alt_labels $_chart_perf_sectorsPerHour_data $_chart_perf_minutesPerSector_data $_chart_rewards_data $_process_disk_data_js_arr 'Farm Performance'
	...
	foreach ($_header in $_process_header_arr)
	{
			...
			if ($_process_state.toLower() -eq "running") {
				$_farmer_status += "<div class=process_tile>"
			}
			else{
				$_farmer_status += "<div class=process_tile_red>"
			}
			$_farmer_status += "<h3 class=chart_font_header>" + $_process_isOftype + ": " + $_process_alt_name + "</h3>"
			$_farmer_status += "<p class=chart_font>" + $_process_state + "</p>"
			if ($_process_isOftype.toLower() -eq "node") {
				$_farmer_status += "<p class=chart_font></p>"
			}
			else {
				$_farmer_status += "<p class=chart_font>" + "Uptime: " + $_process_uptime + "</p>"
			}
			$_farmer_status += "</div>"
	}
	$_farmer_status += "</tr></table>"
	$_html_full += "<div id=container>" + $_farmer_status + "</div>"
	$_html_full += "<br>"
	$_html_full += "<br>"
	$_html_full += "<br>"
	$_html_full += "<br>"
