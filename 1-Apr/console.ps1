...
function fGetSummaryDataForConsole ([array]$_io_process_arr) {
	...
	$_num_cols = $_label_total_length + $_label_separator_count
	#
	#
	...
	foreach ($_header in $_process_header_arr)
	{
			...
			#if ($_plotted_size_TiB -ne 0 -and $_plotted_size_TiB -ne "-" -and $_process_total_TiB_per_day_disp -ne "-" -and $_process_total_TiB_per_day_disp -ne 0)
			#{
...
function fWriteDetailDataToConsole ([array]$_io_farmers_ip_arr) {
	...
	for ($arrPos = 0; $arrPos -lt $_io_farmers_ip_arr.Count; $arrPos++)
	{
		...
		if ($_io_farmers_ip_arr[$arrPos].toString().Trim(' ') -ne "" -and $_io_farmers_ip_arr[$arrPos].toString().IndexOf("#") -lt 0) {
			...
			{
				$_disk_plots_remaining_arr_sorted[$_h].ETA = $_eta_hold_ + [double]($_avg_seconds_per_sector) * $_disk_plots_remaining_arr_sorted[$_h].AdditionalSectorsForETA * $_disk_plots_remaining_arr_sorted[$_h].PlotCountMultiplier
